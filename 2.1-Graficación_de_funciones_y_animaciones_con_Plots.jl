...
# ╔═╡ 6adc3398-c314-4760-927e-02cc67861d0a
@bind d Slider(0.01:0.04:2, default=0.5)

# ╔═╡ 32387b91-9337-40af-a740-158a67552cd5
begin
	plot(sin,-π:d:π, title = "Funciones trigonométricas", xlabel = "x", ylabel = "y", color = "turquoise", label = "Sin(x)")
	plot!(cos,-π:d:π, color = "lightpink3", label = "Cos(x)")
end
...
# ╔═╡ eb57b21d-36ac-4bc7-8e8f-ef38967b3c85
Time=(r*sin(θ)+sqrt(r^2*(sin(θ))^2+2*G*H))/(G) #Tiempo que tarda en tocar el piso

# ╔═╡ 3f7b2754-9032-4f9d-a4c2-bffa64ddf222
	T=r*cos(θ)*Time #Alcance del tiro, es decir, cuando la partícula toca el suelo

# ╔═╡ 4e60883b-dff2-46dc-9f17-59ff7089e2a6
begin
	scatter(P,0:T, title = "Tiro parabólico", xlabel = "x", ylabel = "y", color = "indianred", label = "Recorrido")
end

# ╔═╡ 66591f6e-8c4f-4ec7-9afc-41fd5ec1dc0d
M=H+(r^2*(sin(θ))^2)/(2*G) #Altura máxima

# ╔═╡ 9fd90071-9e87-43a0-870d-630139da8180
begin
	...
	anima = @animate for i in 1:n # para t ∈ 0:n
		scatter!([xpos[i]], [ypos[i]], legend=:none, title="Trayectoria", markersize=2, markercolor="navajowhite", xaxis=("x", (0,T+T/10)), yaxis=("y", (0, M+M/10)), dpi=300) 
	end	
		gif(anima,"aminaciontiro.mp4",fps=30)
	end
end
...
# ╔═╡ a3c5592b-68ec-4a65-87c6-288b1e50d2d8
begin
	X = Y = range(-2π, 2π, length = 40) #Definimos a X y Y
	Z(X,Y)=sin(X)+cos(Y) # Funcioón, no usé h porque ya se había definido antes
	ani = Plots.Animation()
	begin
	ani = @animate for t in -2π:0.5:2π 
		plot!(X, Y, Z, st = [:surface])
		Plots.frame(ani) #Anima de menos 2pi a 2pi, grafica con el arreglo X,Y,Z, muestra un mapa de color
	end
